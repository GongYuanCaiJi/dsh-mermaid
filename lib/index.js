 ...
 */
async function renderBlocksFor(sessionId, turn, text, options = {}) {
    ...
    for (const [index, block] of blocks.slice(0, MAX_BLOCKS).entries()) {
        ...
        if (!normalized) {
            ...
            notify(`dsh-mermaid can't render type "${typeLabel}"${blockLabel}. Supported: ${SUPPORTED_TYPE_LABEL}.`, 'info');
            continue;
        }
